...
\begin{groupIzq}{2.5em}
  ...
  \begin{groupIzq}{1em}
    ...
    \par Tenemos que: \ensuremath{k \cdot n \geq j \cdot n \cdot log(n) \Leftrightarrow \frac{k}{j} \leq log(n)}
    \par Claramente la desigualdad no se va a cumplir para todo \ensuremath{n} puesto que \ensuremath{\frac{k}{j}} es constante.
    \par \ensuremath{\limn log(n) = \infty}, y por definición de límite, se tiene que
    \par \ensuremath{\exists \alpha_0 \in \nat \ tq\ \forall n > \alpha_0 : log(n) > \frac{k}{j} (\frac{k}{j} > 0)}.
  \end{groupIzq}
